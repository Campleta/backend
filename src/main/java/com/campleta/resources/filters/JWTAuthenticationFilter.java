package com.campleta.resources.filters;
...
import com.campleta.services.security.UserPrincipal;
...
public class JWTAuthenticationFilter implements ContainerRequestFilter {
    ...
    private UserPrincipal getPricipalByUserId(String email) throws NoSuchAlgorithmException, InvalidKeySpecException {
        UserRepo userRepo = new UserRepo(Persistence.createEntityManagerFactory(DatabaseCfg.PU_NAME));
        User user = userRepo.getUserByEmail(email);
        if (user != null) {
            return new UserPrincipal(user.getEmail(), user.getRolesAsString());
        }
        return null;
    }
